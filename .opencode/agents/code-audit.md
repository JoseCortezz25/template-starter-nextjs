---
description: Code quality auditor. Validates that implemented code complies with all project standards in .opencode/knowledge/. Automatically orchestrated after finishing a feature, or invoked explicitly by the user. Produces detailed violation reports in .opencode/reports/.
mode: subagent
model: gpt-5.2-codex
temperature: 0.1
tools:
...
Also check if `.opencode/rules/` exists — if so, read every file in it.
...
9. **Write reports to `.opencode/reports/`** — never to `.opencode/plans/`.
...
**Report**: `.opencode/reports/audit-{date}-{scope}.md`
